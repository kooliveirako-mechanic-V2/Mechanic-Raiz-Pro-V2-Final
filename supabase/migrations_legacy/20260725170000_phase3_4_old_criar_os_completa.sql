...
-- Pre-patch MD5: 7aa3de563188a8a49ee82923b0c6cbe2 (6568 bytes)
-- Post-patch MD5: 72102685eff7628f3cdd5de995ce7edc
-- Adicionado guard (oficina, cliente, veículo) + bypass service_role no topo do BEGIN.
-- SET search_path TO public, pg_temp.
-- REVOKE PUBLIC, anon; GRANT authenticated, service_role.
-- Lógica funcional (rate limit, itens, finalizar_os_atomica, veiculos.km_atual) preservada.
...
BEGIN
  IF COALESCE(auth.role(), '') <> 'service_role' THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE id = p_veiculo_id AND oficina_id = p_oficina_id AND cliente_id = p_cliente_id
    ) THEN
...
REVOKE EXECUTE ON FUNCTION public.criar_os_completa(uuid,uuid,uuid,text,text,integer,uuid,date,time without time zone,text,numeric,numeric,boolean,integer,text,uuid,integer,text,jsonb,text,boolean,boolean,boolean,boolean,text[],text,text,text,boolean,boolean,boolean,text,text[],text,text[],integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.criar_os_completa(uuid,uuid,uuid,text,text,integer,uuid,date,time without time zone,text,numeric,numeric,boolean,integer,text,uuid,integer,text,jsonb,text,boolean,boolean,boolean,boolean,text[],text,text,text,boolean,boolean,boolean,text,text[],text,text[],integer) TO authenticated, service_role;
