...
BEGIN
    ...
    WITH os_base AS (
        -- Precisamos somar os itens primeiro porque a OS não tem coluna valor_total
        SELECT 
            os.id,
            os.desconto,
    ...
    vendas_balcao_filtradas AS (
        SELECT 
            ...
            COALESCE((SELECT SUM(valor) FROM movimentacoes_financeiras WHERE venda_balcao_id = v.id AND tipo = 'entrada'), 0) as valor_pago,
            0 as desconto, -- Venda balcão na estrutura atual parece já ser líquida ou desconto não está claro na tabela pai
    ...
    os_filtradas AS (
        SELECT 
            ...
            COALESCE((SELECT SUM(valor) FROM movimentacoes_financeiras WHERE ordem_servico_id = os.id AND tipo = 'entrada'), 0) as valor_pago
        FROM os_base os
    ),
