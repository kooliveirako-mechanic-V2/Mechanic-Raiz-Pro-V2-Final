...
BEGIN
    ...
    -- 2. FATURAMENTO VENDAS BALCÃO E CUSTO (Via itens_venda_balcao)
    SELECT 
        COALESCE(SUM(vb.valor_total), 0),
        COALESCE((
            SELECT SUM(COALESCE(ivb.quantidade, 1) * COALESCE(ivb.custo_unitario, 0))
            FROM itens_venda_balcao ivb
            WHERE ivb.venda_id = vb.id
        ), 0)
    INTO v_vendas_balcao_total, v_vendas_balcao_custo
    FROM vendas_balcao vb
    WHERE vb.oficina_id = p_oficina_id
      AND DATE(vb.created_at) >= p_data_inicio
      AND DATE(vb.created_at) <= p_data_fim;
