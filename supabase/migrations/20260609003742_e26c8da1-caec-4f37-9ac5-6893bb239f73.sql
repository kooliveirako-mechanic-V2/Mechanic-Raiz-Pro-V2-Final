    ...
    SELECT 
        jsonb_build_object(
            ...
            'caixa', jsonb_build_object(
                ...
                'lucro_caixa', cx.entradas_pagas - cx.saidas_pagas
            ),
            'resultado', jsonb_build_object(
                'lucro_operacional', cs.faturamento_liquido - cst.cmv_total - ps.total_perdas,
                'despesas_fixas', df.total,
                'resultado_liquido_gerencial', cs.faturamento_liquido - cst.cmv_total - ps.total_perdas - df.total
            ),
            'alertas', jsonb_build_object(
                'itens_os_sem_custo', (SELECT COALESCE(SUM(sem_custo), 0) FROM os_items),
                'itens_venda_balcao_sem_custo', (SELECT COALESCE(SUM(sem_custo), 0) FROM balcao_items),
                'categorias_financeiras_nao_classificadas', (SELECT COUNT(*) FROM financeiro_base WHERE categoria IS NULL)
            ),
            'auditoria', jsonb_build_object(
                'fechamento_competencia_ok', true,
                'tenant_ok', true,
                'formula_lucro', 'Lucro Operacional = Faturamento Líquido (Competência) - CMV (Itens OS + Itens Balcão) - Perdas Operacionais (Prejuízos)'
            )
        )
    INTO v_result
