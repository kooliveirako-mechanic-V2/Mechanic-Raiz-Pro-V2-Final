...
BEGIN
    ...
    auditoria_ignorados AS (
        -- OS ignoradas
        SELECT 'OS' as tipo, numero::text, id, (valor_servico + (SELECT SUM(COALESCE(it.valor_total, 0)) FROM itens_os it WHERE it.ordem_servico_id = os.id) - COALESCE(desconto, 0)) as valor_liquido, (SELECT SUM(COALESCE(it.custo_unitario, 0) * it.quantidade) FROM itens_os it WHERE it.ordem_servico_id = os.id) as cmv, 0 as caixa, 'teste manifesto' as motivo
        FROM ordens_servico os WHERE id = ANY(v_ids_os_teste)
        UNION ALL
        -- Vendas ignoradas
        SELECT 'Venda' as tipo, numero::text, id, valor_total as valor_liquido, (SELECT SUM(COALESCE(it.custo_unitario, 0) * it.quantidade) FROM itens_venda_balcao it WHERE it.venda_id = v.id) as cmv, valor_total as caixa, 'teste manifesto' as motivo
        FROM vendas_balcao v WHERE id = ANY(v_ids_vendas_teste)
    )
    SELECT jsonb_build_object(
        ...
        'auditoria', jsonb_build_object(
            'registros_ignorados_por_manifesto', (SELECT jsonb_agg(to_jsonb(t)) FROM auditoria_ignorados t),
...
GRANT EXECUTE ON FUNCTION public.get_financeiro_v2_preview_limpeza(UUID, DATE, DATE) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_financeiro_v2_preview_limpeza(UUID, DATE, DATE) TO service_role;