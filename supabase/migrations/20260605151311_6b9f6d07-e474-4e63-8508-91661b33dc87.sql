...
BEGIN
    ...
    vendas_balcao_filtradas AS (
        SELECT 
            ...
            COALESCE((SELECT SUM(valor) FROM financeiro WHERE id = v.financeiro_id AND tipo = 'entrada' AND status = 'pago'), 0) as valor_pago,
            0 as desconto,
            v.created_at::date as data_referencia
    ...
    os_filtradas AS (
        SELECT 
            ...
            COALESCE((SELECT SUM(valor) FROM financeiro WHERE ordem_servico_id = os.id AND tipo = 'entrada' AND status = 'pago'), 0) as valor_pago
        FROM os_base os
    ),
