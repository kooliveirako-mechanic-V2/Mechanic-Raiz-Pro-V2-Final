CREATE OR REPLACE FUNCTION public.get_financeiro_resumo(
  p_oficina_id uuid,
  ...
  SELECT json_build_object(
    ...
    'mes_atual', (
      SELECT json_build_object(
        ...
        -- Lucro Real = (Soma MO) + (Soma Peças - Soma Custo Peças na OS) - Saídas Operacionais
        -- Simplificando para o Dashboard: Entradas (MO + Peças) - Saídas
        -- Mas para Lucro Real de OS: v_lucro_os = valor_servico - custo_servico
        'lucro', COALESCE(SUM(CASE WHEN f.tipo = 'entrada' THEN f.valor ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN f.tipo = 'saida' THEN f.valor ELSE 0 END), 0)
      )
      FROM financeiro f
      WHERE f.oficina_id = p_oficina_id
    ...
    'mensal', (
      SELECT COALESCE(json_agg(
        json_build_object(
          ...
          'lucro', COALESCE(m.entradas, 0) - COALESCE(m.saidas, 0)
