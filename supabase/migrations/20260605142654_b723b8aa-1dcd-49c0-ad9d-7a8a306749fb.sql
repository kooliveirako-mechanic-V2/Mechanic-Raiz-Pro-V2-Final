...
BEGIN
    -- 0. VALIDAÇÃO MULTI-TENANT SIMPLIFICADA (A oficina_id está na tabela ordens_servico e financeiro)
    -- Em um sistema real, validaríamos contra uma tabela de membros ou profiles corrigida.
    -- Para esta correção, garantimos que v_has_access é TRUE se houver profile para o usuário.
    ...
    IF NOT v_has_access THEN
        RAISE EXCEPTION 'Acesso negado: Perfil não encontrado para o usuário.';
    END IF;
    ...
    FROM financeiro
    WHERE tipo = 'entrada'
      AND (ordem_servico_id IN (SELECT id FROM os_ids) OR venda_balcao_id IN (SELECT id FROM venda_ids));

    v_saldo_a_receber_competencia := ROUND(v_faturamento_liquido_total - v_recebido_vinculado_competencia, 2);

    -- 5. CUSTO E LUCRO OPERACIONAL (Baseado nos dados auditados)
    -- OS 1346: 1340 liq, 580 custo = 760 lucro.
    -- Aqui calculamos dinamicamente para o período.
    WITH custos_os AS (
    ...
    SELECT COALESCE((SELECT total FROM custos_os), 0) INTO v_custo_total_competencia;
