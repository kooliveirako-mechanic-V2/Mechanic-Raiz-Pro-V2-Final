-- =====================================================
-- RPC 1: CRIAR OS COMPLETA (ATÔMICA)
-- =====================================================
...
BEGIN
  ...
  -- Determine insert status: always 'pendente' first if finalizing
  -- because trigger baixar_estoque_os only fires on UPDATE to 'finalizado'
  v_final_status := p_status;
  IF p_status = 'finalizado' THEN
    v_insert_status := 'pendente';
  ELSE
  ...
  -- STEP 2: Insert ALL items atomically
  ...
  LOOP
    ...
    BEGIN
      ...
      INSERT INTO public.itens_os (
        ...
        v_unit_price, v_labor, v_item_total,
        v_estoque_id
      );
  ...
  -- STEP 4: If finalizing, UPDATE status (triggers baixar_estoque_os)
...
DECLARE
  ...
  v_item JSONB;
  v_mao_de_obra_apenas NUMERIC;
BEGIN
  -- Lock the OS row to prevent concurrent finalization
  SELECT * INTO v_os
  ...
  SELECT 
    COALESCE(SUM(COALESCE(valor_total, quantidade * valor_unitario, 0)), 0),
  ...
  -- valor_servico on the OS = global labor. Items labor is separate.
  -- Final total = OS.valor_servico (global labor) + all item totals
  v_mao_de_obra_apenas := COALESCE(v_os.valor_servico, 0);
  v_valor_total := v_mao_de_obra_apenas + v_total_itens;

  -- If valor_total is 0 but there was already a valor_servico set, use that
  IF v_valor_total <= 0 AND COALESCE(v_os.valor_servico, 0) > 0 THEN
    v_valor_total := v_os.valor_servico;
    v_mao_de_obra_apenas := v_os.valor_servico;
  END IF;

  -- Update OS to finalized (triggers baixar_estoque_os)
  UPDATE public.ordens_servico SET
    status = 'finalizado',
    ...
    lucro = v_valor_total - COALESCE(v_os.custo_servico, 0),
