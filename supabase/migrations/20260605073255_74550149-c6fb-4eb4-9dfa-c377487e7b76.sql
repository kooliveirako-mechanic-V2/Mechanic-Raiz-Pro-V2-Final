...
BEGIN
    -- 1. FATURAMENTO VENDAS BALCÃO (Peças)
    ...
    custos_vendas AS (
        SELECT SUM(ROUND(COALESCE(ivb.quantidade, 1) * COALESCE(ivb.custo_unitario, 0), 2)) as custo_total
    ...
    -- 3. CLASSIFICAÇÃO DOS ITENS DE OS
    ...
    -- 4. RATEIO DE DESCONTO COM AJUSTE DE CENTAVO
    IF v_faturamento_bruto > 0 THEN
        ...
        -- Ajuste de centavo residual na categoria de maior valor (geralmente peças)
        ...
        IF v_valor_pecas_bruto >= v_valor_servicos_bruto AND v_valor_pecas_bruto >= v_valor_nao_classificado_bruto THEN
            v_valor_pecas_liquido := v_valor_pecas_liquido + v_diferenca_centavos;
        ELSIF v_valor_servicos_bruto >= v_valor_pecas_bruto AND v_valor_servicos_bruto >= v_valor_nao_classificado_bruto THEN
            v_valor_servicos_liquido := v_valor_servicos_liquido + v_diferenca_centavos;
        ELSE
            v_valor_nao_classificado_liquido := v_valor_nao_classificado_liquido + v_diferenca_centavos;
        END IF;
    END IF;
    ...
    FROM financeiro
    WHERE oficina_id = p_oficina_id AND data >= p_data_inicio AND data <= p_data_fim;
    ...
    IF v_total_itens_livres_sem_custo > 0 THEN v_alerta_itens_sem_custo := TRUE; END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'auditoria', jsonb_build_object('total_itens_livres', v_total_itens_livres, 'total_itens_livres_sem_custo', v_total_itens_livres_sem_custo, 'alerta_itens_sem_custo', v_alerta_itens_sem_custo)
    );
END;
$$;