-- Ajustar upsert_financeiro_os para usar casting correto para status_pagamento
...
DECLARE
  ...
  v_existing_id uuid;
  v_os_numero integer;
  v_status_financeiro public.status_pagamento := 'pago'::public.status_pagamento;
BEGIN
  -- Se não informou forma de pagamento, nasce como a_receber
  IF p_forma_pagamento_id IS NULL THEN
    v_status_financeiro := 'a_receber'::public.status_pagamento;
  END IF;
  ...
  INSERT INTO public.financeiro (
    ...
    p_oficina_id, p_ordem_servico_id, 'entrada', 'servico', v_valor_restante, v_status_financeiro,
...
-- Ajustar criar_venda_balcao para usar casting correto
...
BEGIN
  ...
  IF v_total > 0 THEN
    INSERT INTO financeiro (
      ...
      descricao, data_vencimento, data_pagamento, forma_pagamento_id, venda_balcao_id
    ) VALUES (
      p_oficina_id, 'entrada', 'venda_balcao', v_total, 'pago'::public.status_pagamento,
      'Venda Balcão #' || v_numero, CURRENT_DATE, now(), p_forma_pagamento_id, v_venda_id
