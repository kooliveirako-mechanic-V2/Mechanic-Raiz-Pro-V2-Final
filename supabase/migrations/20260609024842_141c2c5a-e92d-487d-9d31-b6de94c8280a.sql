-- 1. Recriar a função upsert_financeiro_os com assinatura única e casting correto
...
-- 2. Corrigir tipagem na fn_tg_reabrir_os e fn_tg_cancelar_os
...
BEGIN
    IF OLD.status = 'finalizado' AND NEW.status != 'finalizado' THEN
        ...
        VALUES (OLD.oficina_id, 'ordem_servico', OLD.id, 'reabertura', jsonb_build_object('status', OLD.status, 'numero', OLD.numero, 'valor_servico', OLD.valor_servico, 'data_reabertura', now()));

        UPDATE public.financeiro
        SET observacoes_contador = COALESCE(observacoes_contador, '') || ' [OS #' || OLD.numero || ' REABERTA - Pagamento mantido]', updated_at = now()
        WHERE ordem_servico_id = OLD.id AND status = 'pago'::public.status_pagamento;

        UPDATE public.financeiro
        SET status = 'cancelado'::public.status_pagamento, observacoes_contador = COALESCE(observacoes_contador, '') || ' [Cancelado por reabertura OS #' || OLD.numero || ']', updated_at = now()
...
BEGIN
    IF NEW.status = 'cancelado' AND OLD.status != 'cancelado' THEN
        UPDATE public.financeiro
        SET status = 'cancelado'::public.status_pagamento, observacoes_contador = COALESCE(observacoes_contador, '') || ' [OS CANCELADA #' || OLD.numero || ']', updated_at = now()
        WHERE ordem_servico_id = OLD.id AND status = 'a_receber'::public.status_pagamento;
        
        INSERT INTO public.log_financeiro_estoque_audit (oficina_id, entidade_tipo, entidade_id, acao, dados_anteriores)
        VALUES (OLD.oficina_id, 'ordem_servico', OLD.id, 'cancelamento', jsonb_build_object('status', OLD.status, 'valor', OLD.valor_servico));
    END IF;
    RETURN NEW;
END;
$function$;
