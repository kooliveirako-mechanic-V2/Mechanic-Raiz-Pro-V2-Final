-- 1. Adicionar coluna de persistência no perfil
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS last_oficina_id UUID REFERENCES public.oficinas(id);

-- 2. Função helper para verificar se é administrador da plataforma
...
BEGIN
    RETURN EXISTS (
        ...
        AND role IN ('master', 'platform_admin') 
        AND active = true
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 3. Atualizar a RPC com a trava de segurança inteligente (Dono ou Admin)
CREATE OR REPLACE FUNCTION public.get_metrics_financeiras_unificadas(
    p_oficina_id uuid,
    p_data_inicio date DEFAULT (date_trunc('month'::text, now()))::date,
    p_data_fim date DEFAULT (date_trunc('month'::text, (now() + '1 mon'::interval)))::date
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
