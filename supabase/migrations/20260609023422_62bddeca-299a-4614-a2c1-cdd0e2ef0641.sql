...
DECLARE
    ...
    v_result JSONB;
BEGIN
    -- Reutiliza a lógica centralizada de métricas (Fonte Única da Verdade)
    v_metrics := get_metrics_financeiras_unificadas(p_oficina_id, p_inicio, p_fim);

    -- Se houve erro no acesso
    IF v_metrics->>'error' IS NOT NULL THEN
        RETURN v_metrics;
    END IF;

    -- Constrói o resultado final mapeado para o contrato UnifiedPreFiscalMetrics
    v_result := jsonb_build_object(
        ...
        'oficina', (
            ...
            FROM oficinas WHERE id = p_oficina_id
        ),
        ...
        'analitico', (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    'Venda Balcão #' || substring(v.id::text from 1 for 8) || ' - ' || COALESCE(v.cliente_nome, 'Consumidor') as descricao,
                ...
                -- Movimentações Financeiras (Saídas e Entradas Diretas)
                SELECT 
                    f.id::text,
