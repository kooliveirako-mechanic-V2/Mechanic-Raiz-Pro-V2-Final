...
BEGIN
    ...
    caixa_sum AS (
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN tipo = 'despesa' THEN valor ELSE 0 END), 0) as saídas
    ...
    SELECT jsonb_build_object(
        ...
        'competencia', jsonb_build_object(
            ...
            'saldo_a_receber', ((SELECT liq FROM os_sum) + (SELECT liq FROM vendas_sum)) - (SELECT entradas FROM caixa_sum)
        ),
