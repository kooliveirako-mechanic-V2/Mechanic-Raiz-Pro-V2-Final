...
BEGIN
    ...
    auditoria_ignorados AS (
        ...
        SELECT 
            ...
            os.numero::int, 
            os.id, 
            (os.valor_servico + (SELECT COALESCE(SUM(it.valor_total), 0) FROM itens_os it WHERE it.ordem_servico_id = os.id) - COALESCE(os.desconto, 0))::numeric as valor_liquido, 
            (SELECT COALESCE(SUM(it.custo_unitario * it.quantidade), 0) FROM itens_os it WHERE it.ordem_servico_id = os.id)::numeric as cmv, 
            ((os.valor_servico + (SELECT COALESCE(SUM(it.valor_total), 0) FROM itens_os it WHERE it.ordem_servico_id = os.id) - COALESCE(os.desconto, 0)) - (SELECT COALESCE(SUM(it.custo_unitario * it.quantidade), 0) FROM itens_os it WHERE it.ordem_servico_id = os.id))::numeric as lucro,
            ...
            ((os.valor_servico + (SELECT COALESCE(SUM(it.valor_total), 0) FROM itens_os it WHERE it.ordem_servico_id = os.id) - COALESCE(os.desconto, 0)) - COALESCE((SELECT SUM(f.valor) FROM financeiro f WHERE f.ordem_servico_id = os.id AND f.status::text = 'pago' AND f.id = ANY(v_ids_financeiro_teste)), 0))::numeric as saldo_a_receber_ignorado,
