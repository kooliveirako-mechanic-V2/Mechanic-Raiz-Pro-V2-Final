...
BEGIN
  -- Criar a venda já como concluida (ou pendente se a tabela permitisse, mas não permite)
  ...
  LOOP
    ...
    -- Atualização direta de estoque para garantir integridade atômica
    IF v_estoque_id IS NOT NULL THEN
      ...
      -- O trigger baixar_estoque_venda_balcao irá apenas registrar a movimentação
    END IF;
